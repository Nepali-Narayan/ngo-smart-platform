...
export default async function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <h1 className="mt-2 text-3xl font-black">Edit Post</h1>
      <p className="mt-2 text-slate-600">Update the record and select an image from your media library.</p>
      <div className="mt-8"><ContentForm table="posts" record={recordResult.data} media={mediaResult.data ?? []} title="Post" /></div>
    </>
  );
}
